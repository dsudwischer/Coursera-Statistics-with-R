...
print("The 'typical' size is usually considered to be represented best by the median which is extremly robust to skewness. However, we will not be able to infer confidence intervals for the median with our framework of the CLT and t-tests, so I will calculate the mean as well.")
median(samp$area)
mean(samp$area)
```
...
# type your code for the Exercise here, and Knit
params$mu >= conf_interval$lower && params$mu <= conf_interval$upper
```
...
```{r calculate-50-cis}
repeated_sample <- ames %>%
        rep_sample_n(size = n, reps = 50, replace = TRUE)
ci <- repeated_sample %>% summarise(lower = mean(area) - z_star_95 * (sd(area) / sqrt(n)),
                  upper = mean(area) + z_star_95 * (sd(area) / sqrt(n)))
```
...
<b>47 out of 50 intervals include the true mean, this is 94%. There is a slight difference that can be explained by randomness. On Average, 95% of confidence intervals include the true mean. If I were to repeat the whole process multiple times and calculate this ration for each repetition, the average percentage of confidence intervals including the true mean would be close to 95%. ALso, there is slight bias due to using the estimated standard deviation of the population instead of the true standard deviation. Consequently, the (normalized) sample mean is t-distributed with 59 DoF. However, a t-distribution with 5 DoF is very close to a standard normal.</b>
...
ci <- repeated_sample %>%
  summarise(lower = mean(area) - z_star_99 * se, upper = mean(area) + z_star_99 * se)
...
ci %>%
  group_by(capture_mu) %>%
  summarise(proportion = n() / dim(ci)[1])
```
