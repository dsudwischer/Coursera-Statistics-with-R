...
complete_nc_data <- nc[complete.cases(select(nc, habit, weight)), ]
ggplot(complete_nc_data, aes(x = habit, y = weight)) +
  geom_boxplot()
```
<b>I will, additionally, use a histogram to better understand the data.</b>
```{r}
ggplot(complete_nc_data, aes(x = weight)) +
  facet_grid(. ~ habit) +
...
<b>We cannot easily check the independence assumption since we do not know whether random sampling was used. If this is the case. The data is not paired, but there might be a few cases of mothers who gave birth two times in 2004 and started smoking in 2004 so that they would appear in both groups. Before doing a real research project, both possible problems should be considerer. To infer insights from the data, we have to make sure that random sampling was used and that the groups are independent.
Also, since we only have 1000 obversation and North Carolina has more than 10000 female inhabitants, the sample comprises less than 10% of the population in question. In particular, both groups are less than 10% of the population. The groups contain 873 and 126 non-NA values respectively, so the sample size is large enough to counter the slight left-skewness of the distributions.</b>

4.  What are the hypotheses for testing if the average weights of babies born to 
smoking and non-smoking mothers are different?
<ol>
<li> <b>This: </b>$H_0: \mu_{smoking} = \mu_{non-smoking}$; $H_A: \mu_{smoking} > \mu_{non-smoking}$ </li>
...
<b>That depends on the confidence level that has been set prior to the analysis. We attain 98% confidence, so we would reject the null hypothesis even at 98% significance.</b>
...
```
<b>The interval is narrower. This was to be expected.</b>
...
inference(y = weight, x = mature, data = nc, statistic = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")
```
<b>The p-value is very large, so the data does not provide strong evidence in favor of the alternative hypothesis. We fail to reject the null hypothesis that there is a difference in average birth weight between younger and mature mothers.</b>
...
```
<b>We will anser this question be calculating the minimum and maximum age of each of the two groups "younger mom" and "mature mom". We observe that the maximum age for younger moms is 34 while the minimum age for mature moms is 35. This seems to be the cut-off value.</b>
...
<b>We will have a look at whether there is a relationship between marital status and weight gained during pregnancy. We start by observing that both groups are large enough to counter slight skewness of the distribution and that both groups are less than 10% of the population.
```{r pick-your-own}
# type your code for the Exercise here, and Knit
table(nc$marital)
```
<b>A simple plot shows that there seems to be little difference in mean weight gained between the two groups.</b>
```{r}
ggplot(data = nc, aes(x = marital, y = gained)) +
  geom_boxplot(na.rm = TRUE)
```
<b>Using the same reasoning for independence as above, we conduct a hypothesis test. We will use a standard 95% significance level, that is, the type 1 error rate should be 95%.</b>
```{r}
inference(y = gained, x = marital, data = nc, statistic = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical", conf_level = 0.95)
```
<b>Since the p-value is high (more than 45%), we fail to reject the null hypothesis. There is no strong evidence that the average weight gain during pregnancy differs by marital status.</b>
