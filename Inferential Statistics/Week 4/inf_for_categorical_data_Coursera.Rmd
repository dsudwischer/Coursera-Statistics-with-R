...
<b><ol><li>Independence of subjects: This assumption seems reasonable based on the methodology paragraph in the article which utilized random sampling. Also, since less than 10% of the world population were surveyed, we can assume indepence between and within groups.</li>
<li>There have to be at least 10 observed successes and 10 observed failures for the data to be nearly normally distributed. In our case, the numbers are 50 and 952, respectively, so this condition as met, too.</li></ol></b> 
...
<b>A margin of error is a tolerance range that allows us to calculate a range of values that we are confident of to include the true proportion rather than just a point estimate that will most certainly not capture the true proportion.</b>
...
n <- dim(us12)[1]
phat <- agg$prop[1]
margin <- z_star * sqrt(phat * (1-phat) / num_obs)
margin
```
...
# I choose Germany and the Netherlands
ger12 <- atheism %>%
  filter(nationality == "Germany", year == 2012)
table(ger12$response)

net12 <- atheism %>%
  filter(nationality == "Netherlands", year == 2012)
table(net12$response)

inference(y = response, data = ger12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")

inference(y = response, data = net12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
<b>The conditions are met again: The indipendence condition holds true for the same reason as for the US and the success-failure-conditions holds true as one can easily see from the table output.</b>
...
<li> <p>The most conservative estimate when calculating a confidence interval occurs when $p$ is set to 1.</p> </li>
</ol>
...
spain <- atheism %>%
  filter(nationality == "Spain", year %in% c(2005, 2012))
inference(y = response, x = as.factor(year), data = spain, statistic = "proportion", type = "ht", method = "theoretical", success = "atheist", alternative = "twosided", null = 0)
```
...
us <- atheism %>%
  filter(nationality == "United States", year %in% c(2005, 2012))
inference(y = response, x = as.factor(year), data = us, statistic = "proportion", type = "ht", method = "theoretical", success = "atheist", alternative = "twosided", null = 0)
```
...
# There are 39 countries in the table. At 95% significance level, the type 1 error rate is 5%.
39 * 0.05
```
...
# The standard error is (assuming the conditions are met) given by sqrt(p * (1 - p) / n).
# Since we have absolutely no guess for p, we use the value of p that maximes the standard error, i.e. p = 0.5.
# This leaves us with the inequality qnorm(0.975) * sqrt(0.5 * 0.5 / n) <= 0.01,
# which is equivalent to n >= (100 * qnorm(0.975) / 2)^2
(100 * qnorm(0.975) / 2)^2
```
