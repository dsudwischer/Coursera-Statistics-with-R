...
ggplot(data = sfo_feb_flights, aes(x = arr_delay)) +
  geom_histogram()
max(sfo_feb_flights$arr_delay) / 60.0
...
late_departments <- nycflights %>%
  filter(dep_type == "delayed") %>%
  mutate(arr_type = ifelse(arr_delay <= 0, "on time", "delayed"))
sum(late_departments$arr_type == "on time") / dim(late_departments)[1]
```