...
<b>I would at least be comfortable enough to build a model and do some diagnostics in evaluate whether the model is useful to construct confidence intervals. Just from a short glance at the plot it does seems reasonable to do so. However, it is difficult to trust the model without having looked at the residual distribution.</b>
...
<li> <b>The relationship is positive, linear, and moderately strong. One of the potential outliers is a team with approximately 5520 at bats.</b> </li>
...
<b>There is no prompt, but since the function appears to call the built-in lm function, I would guess that it automatically calculates the best fit, yielding a sum of squares of 123721.9.</b>
...
<ol>
<li> <b>For each additional home run, the model predicts 1.83 more runs, on average.</b> </li>
...
713 - predict(m1, newdata = data.frame(at_bats = 5579))
```
...
<li> <b>The residuals show a curved pattern.</b> </li> 
...
<li> <b>The residuals are fairly symmetric, with only a slightly longer tail on the right, hence it would be appropriate to deem the the normal distribution of residuals condition met.</b> </li>
</ol>
...
<b>There does indeed seem to be a moderately strong linear positive relationship between bat average and runs.</b>
...
```{r pick-model}
# type your code for the Exercise here, and Knit
m3 <- lm(data = mlb11, formula = runs ~ bat_avg)
summary(m3)
g1 <- ggplot(data = m3, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals")
g2 <- ggplot(data = m3, aes(x = .resid)) +
  geom_histogram(binwidth = 25) +
  xlab("Residuals")
g3 <- ggplot(data = m3, aes(sample = .resid)) +
  stat_qq()
g1
g2
g3
...
<b>The R squared is much higher, indicating that almost two thirds of the total variation can be explained by the model. This finding agrees with my visual perception. To assess the validity of the model, we should take a look at the residuals. The normality and homoscedasticity assumptions seem to be met. Assuming that the true relationship is indeed linear, the model should yield reasonable confidence intervals. However, there are probably much better predictors that explain more of the variability. With a residual standard error of about almost 50, a 95% confidence interval would have a margin of error of almost 100 (in both directions).</b>
...
<li> hits </li>
<li> wins </li>
<li> <b>batting average</b> </li>
</ol>
```{r runs-vs-others}
# type your code for Question 9 here, and Knit
ggplot(data = mlb11, aes(x = homeruns, y = runs)) +
  geom_point() +
  stat_smooth(method = "lm") # there is no plot for runs ~ homeruns, so I will add one here.
...
ggplot(data = mlb11, aes(x = wins, y = runs)) +
  geom_point() +
  stat_smooth(method = "lm")
summary(m2) # for homeruns
# m1 is runs ~ at_bats, m3 is runs ~ avg_bats
...
<b>It seems that the batting average has the best fit with the largest $R^2$ and thus the lowest mean squared error. Also, the residual standard error is comparibily low, promising narrower confidence intervals than the other preditors.</b>
...
<ol>
<li> <b>on-base plus slugging</b> (`new_obs`) </li>
...
ggplot(data = mlb11, aes(x = new_onbase, y = runs)) +
  geom_point() +
  stat_smooth(method = "lm")
m8 <- lm(formula = runs ~ new_slug, data = mlb11)
summary(m8)
m9 <- lm(formula = runs ~ new_onbase, data = mlb11)
summary(m9)
m10 <- lm(formula = runs ~ new_obs, data = mlb11)
summary(m10)
```

<b>On-base + slugging percentage seems to yield the best results, having a large $R^2$ value of about 93% and a low residual standard error that is promising narrow confidence intervals.</b>
...
<b>The normal distrubution of errors seems to be mildly violated. However, due to the low number of samples, this could easibly be due to chance. Heteroscedasticity does not seem to be an issue here. One could do a Chi-squared goodness-of-fit test in order to investigate whether there is significant evidence against the assumption of normality of the residuals.</b> 
<div id="license">
