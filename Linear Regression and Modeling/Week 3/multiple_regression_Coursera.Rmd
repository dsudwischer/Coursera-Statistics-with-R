...
<li> <b>Yes, revise wording to "Is there an association between beauty and course evaluations?"</b> </li>
...
<li> <b>The left skewness of the data suggests that the students are less likely to rate the professors highly.</b> </li>
</ol>
```{r score-dist}
...
ggplot(data = evals, aes(pic_outfit, y = bty_avg)) +
  geom_boxplot()
```
...
<b>In the original plot, multiple points corresponding to the same combination of beauty and score were stacked, making it impossible to determine how many points of that combination actually exist in the data.</b>
...
```{r summary-model}
# type your code for the Exercise here, and Knit
fit <- lm(score ~ bty_avg, data = evals)
summary(fit)
```

<b>The equation is $\hat{score} = 3.88034 + 0.06664 \cdot bty\_avg$. An increase of the beauty average score by 1 unit is expected to be associated with a 0.06664 units increase in the evaluation score, on average.</b> 
...
<li> <b>Nearly normal residuals: Residuals are right skewed, but the sample size is large, so this may not be an important violation of conditions.</b> </li>
...
<b>The observations seem to be be independent, there is no time-series character recognizable. At least if there is only one observation per professor. There might be some slight issues with heteroscedasticity but they appear minor. Also, the residual distribution seems to be left skewed. Overall, these issues appear to be minor.</b>
...
<b>R automatically creates dummy variables for us, exactly one for each level of the factor variable with exception of the reference level (which corresponds to all relevant dummy variables being 0).</b>
...
<li> <b>Tenure Track, Tenured, Teaching</b> </li>
...
<b>I would argue that the proportion of students that participated in the survey should not have any influence on the score. However, there might be some hidden confounding variable that has an association with this proportion.</b>
...
<b>The percentage of students who participated in the survey is actually highly significant. Instead, the number of professors teaching the course section (factor with two levels, "single" and "multiple") is least significant.</b>
...
<li> <b>0.12points higher than minority professors, all else held constant.</b> </li> 
...
<b>The estimates only changed slightly and the p-values are nearly the same as with the full model. So there was probably only slight collinearity.</b>
...
```{r adj-rsq-select}
# type your code for the Exercise here, and Knit
tmp = lm(score ~ rank + ethnicity + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
print(paste("gender", summary(tmp)$adj.r.squared))
tmp = lm(score ~ rank + ethnicity + gender + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
print(paste("language", summary(tmp)$adj.r.squared))
tmp = lm(score ~ rank + ethnicity + gender + language + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
print(paste("age", summary(tmp)$adj.r.squared))
tmp = lm(score ~ rank + ethnicity + gender + language + age 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
print(paste("cls_perc_eval", summary(tmp)$adj.r.squared))
tmp = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
print(paste("cls_students", summary(tmp)$adj.r.squared))
tmp = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
print(paste("cls_profs", summary(tmp)$adj.r.squared))
tmp = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
print(paste("cls_level", summary(tmp)$adj.r.squared))
tmp = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + bty_avg 
             + pic_outfit + pic_color, data = evals)
print(paste("cls_credits", summary(tmp)$adj.r.squared))
tmp = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits 
             + pic_outfit + pic_color, data = evals)
print(paste("bty_avg", summary(tmp)$adj.r.squared))
tmp = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg
             + pic_color, data = evals)
print(paste("pic_outfit", summary(tmp)$adj.r.squared))
tmp = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits  + bty_avg
             + pic_outfit, data = evals)
print(paste("pic_color", summary(tmp)$adj.r.squared))
tmp = lm(score ~ rank + ethnicity + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
print(paste("gender", summary(tmp)$adj.r.squared))
...
<b>Having multiple courses per professor in the data would violate the assumption of independent observations (and thus, residuals would be dependent for courses from different professors). A possible consequence could be that due to omitted variable bias all predictions for a single professor were far off. Consider, for the sake of example, that a professor hands out chocolate during christmas time. This variable would not be captured in the model. But since it would likely affect all courses of that particular professor, the residuals coming from obversations for that professor would likely all point into the same direction. In particular, they would not be independent any.</b>
...
```{r final model}
fm <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(fm)$coef
```

<b>A high evaluation score is expected to come along professors who have high values in the above mentioned categories, i.e. professors who
* are of "teaching" rank
* are not belonging to a minority
* are beautiful
* are male
* are young
* teach upper level, single credit classes
* have a black&white picutre with a formal outfit on the survey
* have large classes with students
* have received feedback from a large proportion of students</b>
...
<b>I would be careful with just generalizing the conclusions. Since the data comes from only one single university, we can only generalize to populations that are similar. For example, in universities with a different demographic structure within the students or the state, the effect of ethnicity or age could be completely different. In order to really generalize the result, random sampling or stratified sampling from similar clusters should have been used.</b>
